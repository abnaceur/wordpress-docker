...
function gridme_grid_cats() {
    if ( 'post' == get_post_type() ) {
        ...
        if ( $categories_list ) {
            ...
            printf( '<div class="gridme-grid-post-categories">' . __( '<span class="gridme-sr-only">Posted in </span>%1$s', 'gridme' ) . '</div>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
    }
}
endif;
...
function gridme_grid_postmeta() { ?>
    ...
    <span class="gridme-grid-post-comment gridme-grid-post-meta"><?php comments_popup_link( sprintf( wp_kses( /* translators: %s: post title */ __( '0 Comment<span class="gridme-sr-only"> on %s</span>', 'gridme' ), array( 'span' => array( 'class' => array(), ), ) ), wp_kses_post( get_the_title() ) ) ); ?></span>
    <?php } ?><?php } ?>
    </div>
    </div>
    <?php } ?>
...
function gridme_single_cats() {
    if ( 'post' == get_post_type() ) {
        ...
        if ( $categories_list ) {
            /* translators: 1: list of categories. */
            printf( '<span class="gridme-entry-meta-single-cats"><i class="far fa-folder-open" aria-hidden="true"></i>&nbsp;' . __( '<span class="gridme-sr-only">Posted in </span>%1$s', 'gridme' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
    }
}
endif;
...
function gridme_single_postmeta() { ?>
    ...
    <span class="gridme-entry-meta-single-comments"><i class="far fa-comments" aria-hidden="true"></i>&nbsp;<?php comments_popup_link( sprintf( wp_kses( /* translators: %s: post title */ __( 'Leave a Comment<span class="gridme-sr-only"> on %s</span>', 'gridme' ), array( 'span' => array( 'class' => array(), ), ) ), wp_kses_post( get_the_title() ) ) ); ?></span>
    <?php } ?><?php } ?>
    <?php if ( !(gridme_get_option('hide_post_categories')) ) { ?><?php gridme_single_cats(); ?><?php } ?>
    <?php if ( !(gridme_get_option('hide_post_edit')) ) { ?><?php edit_post_link( sprintf( wp_kses( /* translators: %s: Name of current post. Only visible to screen readers */ __( 'Edit<span class="gridme-sr-only"> %s</span>', 'gridme' ), array( 'span' => array( 'class' => array(), ), ) ), wp_kses_post( get_the_title() ) ), '<span class="edit-link">&nbsp;&nbsp;<i class="far fa-edit" aria-hidden="true"></i> ', '</span>' ); ?><?php } ?>
    </div>
    <?php } ?>
<?php }
endif;
