<?php
/**
 * Vandana Lite Customizer Notification Section Class.
 *
 * @package Vandana_Lite
 */

/**
 * Vandana_Lite_Customizer_Notice_Section class
 */
class Vandana_Lite_Customizer_Notice_Section extends WP_Customize_Section {
	 ...
	 * @var    string
	 */
	public $type = 'vandana-lite-customizer-plugin-notice-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'vandana_lite_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'vandana_lite_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $vandana_lite_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'vandana_lite_show_recommended_plugins' );

		foreach ( $vandana_lite_recommended_plugins as $slug => $plugin_opt ) {
			...
			$vandana_lite_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$vandana_lite_recommended_plugin['class'] = 'active';
			} else {
				$vandana_lite_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$vandana_lite_recommended_plugin['button_class'] = 'install-now button';
					$vandana_lite_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$vandana_lite_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$vandana_lite_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$vandana_lite_recommended_plugin['button_class'] = 'deactivate-now button';
					$vandana_lite_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$vandana_lite_recommended_plugin['id'] = $slug;
			$vandana_lite_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$vandana_lite_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$vandana_lite_recommended_plugin['description'] = $info->short_description;
			}

			$vandana_lite_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $vandana_lite_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="vandana-lite-recommeded-actions-container vandana-lite-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="vandana-lite-recommeded-actions">
