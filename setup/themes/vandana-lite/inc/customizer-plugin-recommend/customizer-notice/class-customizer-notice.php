<?php
/**
 * Vandana Lite Customizer Notification System.
 *
 ...
 */
class Vandana_Lite_Customizer_Notice {
	...
	/**
	 * The single instance of Vandana_Lite_Customizer_Notice
	 *
	 * @var Vandana_Lite_Customizer_Notice $instance The Vandana_Lite_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Vandana_Lite_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Vandana_Lite_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Vandana_Lite_Customizer_Notice ) ) {
			self::$instance = new Vandana_Lite_Customizer_Notice;
	...
	public function setup_config() {
		...
		$vandana_lite_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$vandana_lite_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'vandana-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'vandana-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), VANDANA_LITE_THEME_VERSION, true );
		wp_localize_script(
			'vandana-lite-customizer-notice', 
            'vandana_lite_customizer_notice_data', 
            array(
				'ajaxurl'            => esc_url( admin_url( 'admin-ajax.php' ) ),
				'template_directory' => esc_url( get_template_directory_uri() ),
				'base_path'          => esc_url( admin_url() ),
				'activating_string'  => __( 'Activating', 'vandana-lite' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Vandana_Lite_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Vandana_Lite_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Vandana_Lite_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'vandana_lite_show_recommended_plugins', $show_recommended_plugins );
		}
