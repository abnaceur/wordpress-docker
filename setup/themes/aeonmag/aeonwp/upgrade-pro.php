...
class AeonMag_Theme_Notice {
	 ...
	 *
	 * AeonMag_Theme_Notice constructor.
	 */
	...
	public static function get_theme_lists() {
		...
		$theme_lists = array(
			'aeonmag'      => 'https://aeonwp.com/aeonmag-plus/',
		);
	...
	public function pro_theme_notice() {
		...
		$option = get_option( 'aeonmag_theme_notice_start_time' );

		if ( ! $option ) {
			update_option( 'aeonmag_theme_notice_start_time', time() );
		}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'aeonmag-notice', get_template_directory_uri() . '/css/upgrade-pro.css', array(), '4.5.0' );
	}
	...
	public function pro_theme_notice_markup() {
		...
		$theme_notice_start_time = get_option( 'aeonmag_theme_notice_start_time' );
		$ignore_notice_permanent = get_user_meta( $this->current_user_data->ID, 'aeonmag_upgrade_pro_theme_notice_ignore', true );
		$ignore_notice_partially = get_user_meta( $this->current_user_data->ID, 'aeonmag_upgrade_pro_theme_notice_partial_ignore', true );
		...
		<div class="notice updated pro-theme-notice">
			<p>
				<?php
				$pro_link = '<a target="_blank" href=" ' . esc_url( $theme_lists[ $current_theme ] ) . ' ">' . esc_html__( 'upgrade to pro', 'aeonmag' ) . '</a>';

				printf(
					esc_html__(
						/* Translators: %1$s current user display name., %2$s Currently activated theme., %3$s Pro theme link., %4$s Coupon code. */
						'Howdy, %1$s! You\'ve been using %2$s theme for a while now, and we hope you\'re happy with it. If you need more options and access to the premium features, you can %3$s. Also, you can use the coupon code %4$s to get 30 percent discount while making the purchase. Enjoy!', 'aeonmag'
					),
			...
			<div class="links">
				<a href="<?php echo esc_url( $theme_lists[ $current_theme ] ); ?>" class="btn button-primary"
				   target="_blank">
					...
					<span><?php esc_html_e( 'Upgrade To Pro', 'aeonmag' ); ?></span>
				</a>

				<a href="?aeonmag_upgrade_pro_theme_notice_partial_ignore=1" class="btn button-secondary">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'aeonmag' ); ?></span>
				</a>
			</div>

			<a class="notice-dismiss" href="?aeonmag_upgrade_pro_theme_notice_ignore=1"></a>
		</div>
	...
	public function pro_theme_notice_partial_ignore() {
		...
		if ( isset( $_GET['aeonmag_upgrade_pro_theme_notice_partial_ignore'] ) && '1' == $_GET['aeonmag_upgrade_pro_theme_notice_partial_ignore'] ) {
			update_user_meta( $user_id, 'aeonmag_upgrade_pro_theme_notice_partial_ignore', time() );
		}
	}
	...
	public function pro_theme_notice_ignore() {
		...
		if ( isset( $_GET['aeonmag_upgrade_pro_theme_notice_ignore'] ) && '1' == $_GET['aeonmag_upgrade_pro_theme_notice_ignore'] ) {
			update_user_meta( $user_id, 'aeonmag_upgrade_pro_theme_notice_ignore', time() );
		}
	}
}
new AeonMag_Theme_Notice();
