...
ob_start();
?>
<h2 class="mt-0"><?php _ex('Marmot Theme - Templates', 'customizer', 'marmot'); ?></h2>
...
if (!defined('\HQExtra\VERSION')) {
    if (!Marmot\is_plugin_installed('hqtheme-extra/hqtheme-extra.php')) {
        ...
        $required_plugins_for_theme_templates .= '<p><a target="_blank" class="button new" href="' . esc_attr($install_url) . '">' . sprintf(_x('Install "%s" plugin', 'settings', 'marmot'), 'HQTheme Extra') . '</a></p>';
    } else {
        ...
        $required_plugins_for_theme_templates .= '<p><a target="_blank" class="button new" href="' . esc_attr($activate_url) . '">' . sprintf(_x('Activate "%s" plugin', 'settings', 'marmot'), 'HQTheme Extra') . '</a></p>';
    }
}
if (!empty($required_plugins_for_theme_templates)) {
    echo '<p>' . sprintf(_x('Theme Templates requires "%s" plugin', 'settings', 'marmot'), 'HQTheme Extra') . '</p>';
    echo $required_plugins_for_theme_templates;
    echo ' <p>' . _x('Customizer refresh is required after plugins activation!', 'settings', 'marmot') . '</p>';
} else {
