 ...
 * Display Breadcrumb
 *
 * @package VIP Business
 */

if ( ! function_exists( 'vip_business_breadcrumb' ) ) :
	/**
	 * Display Breadcrumb
	 * @return html Breadcrumb with links
	 */
	function vip_business_breadcrumb() {
		$show = vip_business_gtm( 'vip_business_breadcrumb_show' );
		...
		$show_on_home = vip_business_gtm( 'vip_business_breadcrumb_show_home' );

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'vip-business' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'vip-business' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'vip-business' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'vip-business' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'vip-business' ); // text for an author page
		$text['404']      = esc_html__( '%1$sError 404%2$s', 'vip-business' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			if ( $show_on_home ) {
				echo '
				<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'vip-business' ) . '" class="breadcrumbs breadcrumb-trail on_home">
			...
			echo '
			<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'vip-business' ) . '" class="breadcrumbs breadcrumb-trail">
						<ol itemscope itemtype="http://schema.org/BreadcrumbList" class="trail-items">';
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'vip-business' ) ) ) ), esc_html( get_the_time( __( 'Y', 'vip-business' ) ) ), $content_position++ ) ;

				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'vip-business' ) ) ) ), esc_html(get_the_time( __( 'F', 'vip-business' ) ) ), $content_position++ );

				echo $before . esc_html( get_the_time( __( 'd', 'vip-business' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'vip-business' ) ) ) ), esc_html( get_the_time( __( 'Y', 'vip-business' ) ) ), $content_position++ ) ;

				echo $before . esc_html( get_the_time( __( 'F', 'vip-business' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'vip-business' ) ) ) . sprintf( $after, $content_position++ );
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'vip-business' ), absint( max( $paged, $page ) ) );
