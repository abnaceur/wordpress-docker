/* global vipBusinessScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(vipBusinessScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: vipBusinessScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(vipBusinessScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === vipBusinessScreenReaderText.expand ? vipBusinessScreenReaderText.collapse : vipBusinessScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.vipBusiness', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.vipBusiness', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.vipBusiness', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').unbind('touchstart.vipBusiness');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.vipBusiness', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.vipBusiness blur.vipBusiness', function() {
  ...
  $(".portfolio-item:even").addClass("even-item");
	$(document).ready(function() {
		$(window)
			.on('load.vipBusiness resize.vipBusiness', function() {
				if (window.innerWidth < 910) {
	...
	jQuery( document ).ready( function() {
		...
		jQuery( window )
			...
			if ( window.innerWidth > 99 ) {
				jQuery('#search-container').on('focusout', function () {
