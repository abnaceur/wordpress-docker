<?php
/**
 * Mixed List Style Widget.
 * HomePage Widget
 * @package Buzz_Magazine
 */

class Buzz_Magazine_Mixed_List extends WP_Widget{
    ...
    function __construct() {
        ...
        parent::__construct(
                'buzz_magazine_mixed_list',
                esc_html__( 'Buzz Magazine: Mixed List Style', 'buzz-magazine' ),
                $widget_ops
        );
    }
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance,
            array(
                'title'			        => esc_html__( 'Business', 'buzz-magazine' ),
                'category'       	    => '',
                'icon'       	        => 'fa-briefcase',
                'display_per_posts'     => 4,
                'display_date'     	    => true,
	    ...
	    $icon                   = isset( $instance['icon'] ) ? esc_html( $instance['icon'] ) : esc_html('fa-briefcase');
        ...
        ?>
        <p>
            <img src="<?php echo esc_url(BUZZ_MAGAZINE_IMAGE_URI. 'widgets/mixed-list.png')?>" alt="mixed-list-style">
        </p>
    ...
    function widget( $args, $instance ) {
        ...
        extract( $args );
        $title 				= ( ! empty( $instance['title'] ) ) ? esc_html($instance['title']) : esc_html__('Business','buzz-magazine');
	    $title              = apply_filters( 'widget_title', $title, $instance, $this->id_base );
	    $category  			= isset( $instance[ 'category' ] ) ? $instance[ 'category' ] : 0;
        $display_per_posts 	= ( ! empty( $instance['display_per_posts'] ) ) ? absint( $instance['display_per_posts'] ) : 4;
	    $icon 			    = ( ! empty( $instance['icon'] ) ) ? esc_html($instance['icon']) : esc_html('fa-briefcase');
        ...
        <?php endif; ?>
        <?php $args = array(
            'posts_per_page' => absint( $display_per_posts + 1 ),
            'post_type'      => 'post',
            'post_status'    => 'publish',
        ...
        $query_mixed_list_style = new WP_Query( $args );

        if ($query_mixed_list_style->have_posts()) : $count= 0; ?>
            <div class="layout-section1-wrap">
                <?php while ( $query_mixed_list_style->have_posts() ) : $query_mixed_list_style->the_post(); $count++;
                    $image_size = 'buzz-magazine-mixed-list-style';
                    if ( $count > 1 ){
