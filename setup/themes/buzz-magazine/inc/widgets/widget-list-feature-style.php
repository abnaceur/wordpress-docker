<?php
/**
 * List Feature Style Widget.
 * HomePage Widget
 * @package Buzz_Magazine
 */

class Buzz_Magazine_List_Widget_Feature_Style extends WP_Widget{

    function __construct() {
        $widget_ops = array(
            'classname'                     => 'layout-section5',
            'description'                   => esc_html__( 'Display List Feature Style Widget.', 'buzz-magazine' ),
            'customize_selective_refresh'   => true,

        );
        parent::__construct(
                'buzz_magazine_list_feature_style',
                esc_html__( 'Buzz Magazine: List Feature Style', 'buzz-magazine' ),
                $widget_ops
        );
    }
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance,
            array(
                'title'			        => esc_html__( 'Travel', 'buzz-magazine' ),
                'category'       	    => '',
                'icon'       	        => 'fa-fighter-jet',
                'display_per_posts'     => 3,
                'display_date'	        => true,
        ...
        $title                  = isset( $instance['title'] ) ? esc_html( $instance['title'] ) : esc_html__( 'Travel', 'buzz-magazine' );
        $category 		    	= isset( $instance['category'] ) ? absint( $instance['category'] ) : 0;
	    $icon                   = isset( $instance['icon'] ) ? esc_html( $instance['icon'] ) : esc_html('fa-fighter-jet');
	    ...
	    $display_per_posts   	= isset( $instance['display_per_posts'] ) ? absint( $instance['display_per_posts'] ) : 3;
        ...
        ?>
        <p>
            <img src="<?php echo esc_url(BUZZ_MAGAZINE_IMAGE_URI. 'widgets/list-feature.png')?>" alt="list-feature">
        </p>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Slider Category:', 'buzz-magazine' ); ?></label>
            <?php
    ...
    function widget( $args, $instance ) {
        ...
        extract( $args );
        $title 				    = ( ! empty( $instance['title'] ) ) ? esc_html($instance['title']) :esc_html__('Travel','buzz-magazine');
	    $title                  = apply_filters( 'widget_title', $title, $instance, $this->id_base );
	    $icon 				    = ( ! empty( $instance['icon'] ) ) ? esc_html($instance['icon']) : esc_html('fa-fighter-jet');
        ...
        $view_all_title 	    = ( ! empty( $instance['view_all_title'] ) ) ? esc_html($instance['view_all_title']) :esc_html__('view all','buzz-magazine');
        ...
        ?>
            ...
            $query_list_feature_style = new WP_Query( $args );

            if ($query_list_feature_style->have_posts()) : $count= 0; ?>
                <div class="layout-section5-wrap">
                    <?php while ( $query_list_feature_style->have_posts() ) : $query_list_feature_style->the_post(); $count++;

                        $total = $query_list_feature_style->post_count;
                        $image_size = 'full';
                        ...
                        <article class="post hentry">
                            ...
                            <div class="post-content">
                                <?php if ( true == $display_category && $count > $total - 1 ):
	                            ...
	                            <?php if ($count <= $total - 1 && $display_content == true):?>
