...
class Buzz_Magazine_Widget_Feature_List_Style extends WP_Widget{

    function __construct() {
        $widget_ops = array(
            'classname'   => 'layout-section4',
            'description' => esc_html__( 'Display Feature List Style Widget.', 'buzz-magazine' ),
            'customize_selective_refresh' => true,

        );
        parent::__construct(
                'buzz_magazine_widget_feature_list_style',
                esc_html__( 'Buzz Magazine: Feature List Style', 'buzz-magazine' ),
                $widget_ops
        );
    }
    ...
    function form( $instance ) {
        ...
        $instance = wp_parse_args( (array) $instance,
            array(
                'title'			        => esc_html__( 'Food', 'buzz-magazine' ),
                'category'       	    => '',
                'display_per_posts'     => 3,
                'icon'       	        => esc_html(' fa-leaf'),
                'display_date'        	=> true,
	    ...
	    $icon                   = isset( $instance['icon'] ) ? esc_html( $instance['icon'] ) : esc_html(' fa-leaf');
        ...
        ?>
        <p>
            <img src="<?php echo esc_url(BUZZ_MAGAZINE_IMAGE_URI. 'widgets/feature-list.png')?>" alt="feature-list">
        </p>
        <p>
    ...
    function widget( $args, $instance ) {
        ...
        extract( $args );
        $title 				    = ( ! empty( $instance['title'] ) ) ? esc_html($instance['title']) :esc_html__('Food','buzz-magazine');
	    ...
	    $icon 				    = ( ! empty( $instance['icon'] ) ) ? esc_html($instance['icon']) : esc_html(' fa-leaf');
        ...
        ?>
            ...
            $feature_list_query = new WP_Query( $args );

            if ($feature_list_query->have_posts()) : $count= 0; ?>
                <div class="layout-section4-wrap">
                    <?php while ( $feature_list_query->have_posts() ) : $feature_list_query->the_post(); $count++;
                        $image_size = 'full';
                        ?>
