...
class milton_lite_notice {
	...
	private function is_dismiss_notice() {
		return apply_filters( 'milton_lite_' . $this->name . '_notice_dismiss', true );
	}
	...
	public function get_notices() {
		...
		$academiathemes_theme_admin_notices = get_option( 'milton_lite_admin_notices' );
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['milton-hide-notice'] ) && isset( $_GET['_milton_lite_notice_nonce'] ) ) {
			
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_milton_lite_notice_nonce'] ), 'milton_lite_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'milton-lite' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'milton-lite' ) );
			}

			if ( $_GET['milton-hide-notice'] ) {
				
				$hide_notice_id = sanitize_text_field( wp_unslash( $_GET['milton-hide-notice'] ) );
				...
				if ( isset($run_update) ) {
					update_option( 'milton_lite_admin_notices', $theme_admin_notices );
				}
