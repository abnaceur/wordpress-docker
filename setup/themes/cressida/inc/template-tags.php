...
if ( ! function_exists( 'cressida_entry_author' ) ) :
	/**
	 * Entry Author
	 * @return string Prints current post author, inside link for archives and without link for singles
	 */
	function cressida_entry_author() {
		$cressida_blog_feed_author_show = cressida_get_option( 'cressida_blog_feed_author_show' );
        $author = get_the_author_meta( 'display_name' );
        if($cressida_blog_feed_author_show && $author != '') {
            echo sprintf(
                '<span class="entry-author"><a href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
                esc_url( get_author_posts_url( get_the_author_meta('ID') ) ),
                esc_attr( sprintf( __( 'View all posts by %s', 'cressida' ), $author ) ),
                $author
            );
        }
	}
endif; // ! function_exists( 'cressida_entry_author' )
