 ...
 * @package Cressida
 */
if ( ! class_exists( 'Cressida_Welcome_Page' ) ) {

	/**
	 * Singleton class used for generating Welcome page for the theme.
	 */
	class Cressida_Welcome_Page {
		/**
		...
		/**
		 * The single instance of Cressida_Welcome_Page
		 *
		 * @var Cressida_Welcome_Page $instance The  Cressida_Welcome_Page instance.
		 */
		private static $instance;

		/**
		 * The Main Cressida_Welcome_Page instance.
		 *
		 * Make sure that only one instance of
		 * Cressida_Welcome_Page exists at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Cressida_Welcome_Page ) ) :
				self::$instance = new Cressida_Welcome_Page;
		...
		public function setup_config() {
			...
			// More theme info.
			$this->theme_pro_uri  = 'https://www.lyrathemes.com/cressida-pro/';
			$this->theme_demo_uri = 'http://www.lyrathemes.com/preview/?theme=cressida';

			// Welcome page info.
			$this->default_title = sprintf( esc_html__( 'About %', 'cressida' ), $this->theme_name );
			...
			$this->welcome_title = isset( $this->config['welcome_title'] ) ? $this->config['welcome_title'] : sprintf( esc_html__( 'Welcome to %', 'cressida' ), $this->theme_name );
			$this->welcome_content = isset( $this->config['welcome_content'] ) ? $this->config['welcome_content'] : $theme->get( 'Description' );

			// Notification content.
			// Translators: 1. Theme name
			$this->notification_welcome = sprintf( esc_html__( 'Welcome! Thank you for choosing %s! ', 'cressida' ), $this->theme_name );
			$this->notification_welcome .= esc_html__( 'To fully take advantage of the best our theme can offer please make sure you visit our ', 'cressida' );
			$this->notification_welcome .= '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome' ) ) . '">' . esc_html__( 'welcome page', 'cressida' ) . '</a>!';
			// Translators: 1. Theme name
			$this->notification_get_started = '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome' ) ) . '" class="button" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'cressida' ), $this->theme_name ) . '</a>';
		...
		public function theme_activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) :
				add_action( 'admin_notices', array( $this, 'cressida_welcome_page_admin_notice' ), 99 );
			endif;
		}
		 ...
		 */
		public function cressida_welcome_page_admin_notice() {
			...
			if ( ! empty( $this->welcome_title ) || ! empty( $this->welcome_content ) || ! empty( $this->tabs ) ) :
				$output = '<div class="wrap welcome-wrap">';
					...
					$output .= '<div class="welcome-intro">';
						$output .= '<a href="' . esc_url($this->theme_uri) . '" target="_blank" class="welcome-screenshot"><img src="' . get_template_directory_uri() . '/screenshot.jpg" ></a>';

						if ( ! empty( $this->welcome_content ) ) :
							$output .= '<p class="welcome-description">';
							$output .= wp_kses_post( $this->welcome_content );
							$output .= '<br /><br /><a class="button button-primary button-large" href="' . esc_url($this->theme_uri) . '" target="_blank"> ' . esc_html__('More Details', 'cressida') . '</a>';
							$output .= '&nbsp;&nbsp;<a class="button button-primary button-large" href="' . esc_url($this->theme_demo_uri) . '" target="_blank"> ' . esc_html__('View Demo', 'cressida') . '</a>';
							$output .= '&nbsp;&nbsp;<a class="button button-primary button-large" href="' . esc_url($this->theme_pro_uri) . '" target="_blank">&raquo; ' . esc_html__('Pro Version', 'cressida') . ' &laquo;</a>';
							$output .= '</p>';
					...
					// Tabs.
					if ( ! empty( $this->tabs ) ) :
						$active_tab = isset( $_GET['tab'] ) ? sanitize_key( $_GET['tab'] ) : 'getting_started';
						// Tabs navigation
		...
		public function getting_started() {
			...
			if ( is_array( $tab_content ) && ! empty( $tab_content ) ) :
				...
				if ( is_array( $tab_content['cards'] ) && ! empty( $tab_content['cards'] ) ) :
					$output .= '<div class="cressida-welcome-cards">';

					foreach ( $tab_content['cards'] as $card ) :
						$output .= '<div class="cressida-welcome-card">';
						if ( $card['title'] ) :
						...
						$output .= '</div><!-- cressida-welcome-card -->';
					endforeach;

					$output .= '</div><!-- cressida-welcome-cards -->';
			...
			if ( is_array( $tab_content ) && ! empty( $tab_content ) ) :
				$output = '<div class="welcome-section welcome-section-free-vs-pro">';
					...
					if ( $tab_content['pro'] && $tab_content['pro_url'] ) :
						// Translators: 1. Theme name
						$output .= '<a href="' . $tab_content['pro_url'] . '" class="button button-primary button-large" target="_blank">' . sprintf( esc_html__( 'Upgrade to %s', 'cressida' ), $tab_content['pro'] ) . '</a>';
						$output .= '&nbsp;&nbsp;<a href="' . $tab_content['compare_url'] . '" class="button button-primary button-large" target="_blank">' . esc_html__( 'Review Full Comparison', 'cressida' ) . '</a>';
					endif;

					$output .= '<div class="cressida-features-table">';

					if ( $tab_content['free'] || $tab_content['pro'] ) :
						$output .= '<div class="cressida-feature-item cressida-feature-item-heading"><strong>' . esc_html__( 'Features', 'cressida' ) . '</strong></div>';
						if ( $tab_content['free'] ) :
							$output .= '<div class="cressida-feature-item-is cressida-feature-item-heading"><strong>' . $tab_content['free'] . '</strong></div>';
						endif;
						if ( $tab_content['pro'] ) :
							$output .= '<div class="cressida-feature-item-is cressida-feature-item-heading"><strong>' . $tab_content['pro'] . '</strong></div>';
						endif;
					endif;
					...
					if ( ! empty( $result ) ) :
						foreach ( $result as $feature ) :
							if ( $feature[0] ) :
								$output .= '<div class="cressida-feature-item"><strong>' . $feature[0] . '</strong></div>';
							endif;
							if ( $feature[1] ) :
								$output .= '<div class="cressida-feature-item-is"><span class="dashicons dashicons-yes"></span></div>';
							else :
								$output .= '<div class="cressida-feature-item-is"><span class="dashicons dashicons-no-alt"></span></div>';
							endif;
							if ( $feature[2] ) :
								$output .= '<div class="cressida-feature-item-is"><span class="dashicons dashicons-yes"></span></div>';
							else :
					...
					$output .= '</div><!-- cressida-features-table -->';
		...
		public function enqueue_style_and_scripts( $hook_suffix ) {
			wp_enqueue_style( 'cressida-welcome-page-css', get_template_directory_uri() . '/welcome-page/css/welcome-page.css', array(), $this->theme_version );
		}
	}
}
