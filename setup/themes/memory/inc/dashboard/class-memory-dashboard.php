...
class Memory_Dashboard {
	...
	public function footer_text() {
		...
		echo wp_kses_post( sprintf( __( 'Please rate <strong>%1$s</strong> <a href="https://wordpress.org/support/theme/%2$s/reviews/" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="https://wordpress.org/support/theme/%2$s/reviews/" target="_blank">WordPress.org</a> to help us spread the word. Thank you from GretaThemes!', 'memory' ), $this->theme->name, $this->slug ) );
	}
		...
		if ( is_admin() && isset( $_GET['activated'] ) && 'themes.php' === $pagenow ) {
			...
			<div class="updated notice notice-success is-dismissible">
				<p>
					...
					echo wp_kses_post( sprintf( __( 'Welcome! Thank you for choosing %1$s. To get started, visit our <a href="%2$s">welcome page</a>.', 'memory' ), $this->theme->name, esc_url( admin_url( 'themes.php?page=' . $this->slug ) ) ) );
					?>
				</p>
				<p>
					<a class="button" href="<?php echo esc_url( admin_url( 'themes.php?page=' . $this->slug ) ); ?>">
						...
						echo esc_html( sprintf( __( 'Get started with %s', 'memory' ), $this->theme->name ) );
						?>
					</a>
				</p>
			</div>
	...
	public function recommended_plugins_action() {
		...
		if ( $plugins_number > 1 ) {
			$action['title'] = esc_html__( 'Install The Recommended Plugins', 'memory' );
			/* translators: theme name. */
			$action['body']        = sprintf( esc_html__( '%s needs some plugins to working properly. Please install and activate our recommended plugins.', 'memory' ), $this->theme->name );
			$action['button_text'] = esc_html__( 'Install Plugins', 'memory' );
		} else {
			$plugin_name = $plugins[0]['name'];
			/* translators: theme name. */
			$action['body'] = sprintf( __( '%1$s needs %2$s to working properly. Please install and activate it.', 'memory' ), $this->theme->name, $plugin_name );
			/* translators: plugin name. */
			$action['button_text'] = sprintf( esc_html__( 'Install %s', 'memory' ), $plugin_name );
		...
		if ( $installer->is_tgmpa_complete() ) {
			if ( $plugins_number > 1 ) {
				$action['body'] = '<strong>' . esc_html__( 'You have installed and active all recommended plugins', 'memory' ) . '</strong>';
			} else {
				/* translators: plugin name. */
				$action['body'] = sprintf( __( '<strong>%s has been installed and activated</strong>', 'memory' ), $plugin_name );
			}
			$action['button_text'] = '';
		}
		return $action;
