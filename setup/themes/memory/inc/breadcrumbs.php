 ...
 */
function memory_breadcrumbs( $args = '' ) {
	if ( is_front_page() ) {
		return;
	}
	...
	$args = wp_parse_args(
		$args,
		array(
			'separator'         => '<i class="icofont icofont-caret-right"></i>',
			'home_label'        => esc_html__( 'Home', 'memory' ),
			'home_class'        => 'home',
	...
	$args = apply_filters( 'memory_breadcrumbs_args', $args );

	$title_position = get_theme_mod( 'post_title_position', 'below-featured-image' );
	...
	if ( is_home() && ! is_front_page() ) {
		...
		if ( 'jetpack-portfolio' === $post_type ) {
			$description = get_theme_mod( 'portfolio_page_description', esc_html__( 'A Short Description Goes Here', 'memory' ) );
		}
	} elseif ( is_single() ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			$term    = current( $terms );
			$terms   = memory_get_term_parents( $term->term_id, $args['taxonomy'] );
			$terms[] = $term->term_id;
		...
		$pages = memory_get_post_parents( get_queried_object_id() );
		...
		$terms        = memory_get_term_parents( get_queried_object_id(), $current_term->taxonomy );
		foreach ( $terms as $term_id ) {
		...
		$title = sprintf( esc_html__( 'Search results for &quot;%s&quot;', 'memory' ), get_search_query() );
	} elseif ( is_404() ) {
		$title = esc_html__( 'Not Found', 'memory' );
	} elseif ( is_author() ) {
		...
		$title = sprintf( esc_html( '%s', 'memory' ), get_the_date() );
	} elseif ( is_month() ) {
		$title = sprintf( esc_html( '%s', 'memory' ), get_the_date( 'F Y' ) );
	} elseif ( is_year() ) {
		$title = sprintf( esc_html( '%s', 'memory' ), get_the_date( 'Y' ) );
	} else {
		$title = esc_html__( 'Archives', 'memory' );
	}
	$items[] = sprintf( $item_text_tpl, $title );
	if ( is_single() ) {
		if ( 'below-header' === $title_position ) {
	...
	echo wp_kses_post( $args['before'] . implode( $args['separator'], $items ) . $args['after'] . $title ); // WPCS: XSS OK.
}
 ...
 */
function memory_get_term_parents( $term_id = '', $taxonomy = 'category' ) {
 ...
 */
function memory_get_post_parents( $post_id = '' ) {
