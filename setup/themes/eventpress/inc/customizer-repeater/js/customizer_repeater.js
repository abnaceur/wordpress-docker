/* global jQuery */
/* global wp */
function eventpress_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function eventpress_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function eventpress_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + eventpress_customizer_repeater_uniqid();
...
function customizer_repeater_refresh_general_control_values() {
    ...
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + eventpress_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== '' || title !== '' || subtitle !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || color !== '') {
                values.push({
                    ...
                    'color': color,
                    'color2': color2,
                    'text': eventpressescapeHtml(text),
                    'link': link,
                    'text2': eventpressescapeHtml(text2),
                    'link2': link2,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
                    'title': eventpressescapeHtml(title),
                    'subtitle': eventpressescapeHtml(subtitle),
                    'social_repeater': eventpressescapeHtml(social_repeater),
                    'id': id,
                    'shortcode': eventpressescapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    ...
    eventpress_media_upload('.customizer-repeater-custom-media-button');
    ...
    theme_conrols.on('click', '.customizer-repeater-new-field', function () {
        ...
        var id = 'customizer-repeater-' + eventpress_customizer_repeater_uniqid();
		...
		if(split_add_more_button_split=="Add New Imag")
        {
        if(jQuery('#exist_eventpress_Image').val()>=8)
        {
        jQuery(".customizer_gallery_upgrade_section").show();
        return false;   
        }
         if(jQuery('#exist_eventpress_Image').val()<8)
        {
         var new_gallery_add_val=parseInt(jQuery('#exist_eventpress_Image').val())+1;
         jQuery('#exist_eventpress_Image').val(new_gallery_add_val);  
        }
        }
    ...
    theme_conrols.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
				...
				if(split_delete_button_split=="Delete Image")
                {
                jQuery('#exist_eventpress_Image').val(parseInt(jQuery('#exist_eventpress_Image').val())-1);  
                }
    ...
    theme_conrols.on('click', '.social-repeater-add-social-item', function (event) {
        ...
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + eventpress_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    theme_conrols.on('click', '.social-repeater-remove-social-item', function (event) {
        ...
        th.remove();
        eventpress_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });
...
function eventpressescapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return eventpressentityMap[s];
    });
