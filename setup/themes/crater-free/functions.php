...
define('CRATER_FREE_REQUIRED_PHP_VERSION', '5.6' );
...
function crater_free_check_theme_setup( $oldtheme_name, $oldtheme ) {
	// Compare versions.
	if ( version_compare(phpversion(), CRATER_FREE_REQUIRED_PHP_VERSION, '<') ) :
	// Theme not activated info message.
	add_action( 'admin_notices', 'crater_free_php_admin_notice' );
	function crater_free_php_admin_notice() {
		?>
			<div class="update-nag">
		  		<?php esc_html_e( 'You need to update your PHP version to a minimum of 5.6 to run Crater Free WordPress Theme.', 'crater-free' ); ?> <br />
		  		<?php esc_html_e( 'Actual version is:', 'crater-free' ) ?> <strong><?php echo phpversion(); ?></strong>, <?php esc_html_e( 'required is', 'crater-free' ) ?> <strong><?php echo CRATER_FREE_REQUIRED_PHP_VERSION; ?></strong>
			</div>
		<?php
	}
...
add_action( 'after_switch_theme', 'crater_free_check_theme_setup', 10, 2  );
...
function crater_free_setup() {
	 ...
	 * If you're building a theme based on Crater Free, use a find and replace
	 * to change 'crater-free' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'crater-free', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'crater-free' ),
		'footer-social' => esc_html__( 'Footer Social Menu', 'crater-free' ),
	) );
...
function crater_free_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Blog Sidebar', 'crater-free' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'crater-free' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Page Sidebar', 'crater-free' ),
		'id'            => 'page',
		'description'   => esc_html__( 'Add widgets here.', 'crater-free' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
	...
	for ( $i=1; $i <= $widget_num; $i++ ) {
		register_sidebar( array(
			'name'          => esc_html__( 'Footer Column', 'crater-free' ) . $i,
			'id'            => 'footer-' . $i,
			'description'   => '',
...
function crater_free_scripts() {
	...
	wp_enqueue_style( 'crater-free-responsive', get_template_directory_uri() . '/css/style-responsive.css', array(), '1.0.5');	   
	...
	wp_enqueue_script( 'crater-free-script', get_template_directory_uri() . '/js/main.js', array('jquery'), '1.0.5', true );		
...
if ( function_exists( 'pll_register_string' ) ) :
	...
	function crater_free_pll_register_string() {
		pll_register_string( 'cr_home_heading_text', esc_html(get_theme_mod( 'cr_home_heading_text' )), 'crater-free', true );
		pll_register_string( 'cr_home_subheading_text', esc_html(get_theme_mod( 'cr_home_subheading_text' )), 'crater-free', true );
		pll_register_string( 'cr_home_button_text', esc_html(get_theme_mod( 'cr_home_button_text' )), 'crater-free', true );
		pll_register_string( 'cr_home_button_url', esc_html(get_theme_mod( 'cr_home_button_url' )), 'crater-free', true );
		pll_register_string( 'cr_home_button2_text', esc_html(get_theme_mod( 'cr_home_button2_text' )), 'crater-free', true );
		pll_register_string( 'cr_home_button2_url', esc_html(get_theme_mod( 'cr_home_button2_url' )), 'crater-free', true );
		pll_register_string( 'cr_copyright_text', esc_html(get_theme_mod( 'cr_copyright_text' )), 'crater-free', true );
	}
