...
/*callback functions header section*/
if ( !function_exists('grip_header_trending_active_callback') ) :
  function grip_header_trending_active_callback(){
      global $grip_theme_options;
      $grip_theme_options = grip_get_options_value();
      $enable_header = absint($grip_theme_options['grip-enable-top-header']);
      $enable_trending = absint($grip_theme_options['grip-enable-trending-news']);
      if( 1 == $enable_header && 1 == $enable_trending   ){
          return true;
      }
      else{
          return false;
...
$wp_customize->add_control(
  new grip_Customize_Category_Dropdown_Control(
    ...
    array(
      ...
      'priority'  => 5,
      'active_callback'=>'grip_header_trending_active_callback'
    )
  )
);
