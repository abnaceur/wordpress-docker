 ...
 * @package Grip
 */
if (!class_exists('Grip_Advertisement_Widget')) :
    /**
     * Advertisement widget class.
     *
     * @since Grip 1.0.0
     */
    class Grip_Advertisement_Widget extends WP_Widget {

        function __construct() {
            $opts  = array(
                'classname'                   => 'grip_advertisement',
                'description'                 => __( 'Small (125*125) Advertisement.', 'grip' ),
                'customize_selective_refresh' => true,
            );
            $control_ops = array( 'width' => 200, 'height' => 250 );
            parent::__construct( false, $name = __( 'Grip(125*125) Advertisement', 'grip' ), $opts );
        }
     ...
     */
        ...
        for ( $i = 1; $i < 5; $i ++ ) {
            $image_link = 'grip_ads_image_link_' . $i;
            $image_url  = 'grip_ads_image_url_' . $i;
    ...
    function update( $new_instance, $old_instance ) {
        ...
        function form( $instance ) {
            // Defaults.
            $defaults = array(
                'title'                => esc_html__('Advertisement', 'grip'),
                'grip_ads_image_url_1'  => '',
                'grip_ads_image_url_2'  => '',
                'grip_ads_image_url_3'  => '',
                'grip_ads_image_url_4'  => '',
                'grip_ads_image_link_1' => '',
                'grip_ads_image_link_2' => '',
                'grip_ads_image_link_3' => '',
                'grip_ads_image_link_4' => '',
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'grip' ); ?></label>
                <input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" />
            </p>
            <label><?php _e( 'Add your Advertisement 125x125 Images Here', 'grip' ); ?></label>
            <?php
            for ( $i = 1; $i < 5; $i ++ ) {
                $image_link = 'grip_ads_image_link_' . $i;
                $image_url  = 'grip_ads_image_url_' . $i;
                ?>
                <p>
                    <label for="<?php echo $this->get_field_id( $image_link ); ?>"> <?php _e( 'Advertisement Image Link ', 'grip' );
                    echo $i; ?></label>
                ...
                <p>
                   <label for="<?php echo $this->get_field_id($image_url); ?>">
                    <?php _e('Upload Advertisement Image', 'grip'); ?>
                </label>
...
add_action('admin_enqueue_scripts', 'grip_ads_widgets_backend_enqueue');
function grip_ads_widgets_backend_enqueue()
{
