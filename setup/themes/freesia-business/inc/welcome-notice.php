 ...
 *
 *
 * @package freesia-business
 */

function freesia_business_admin_notice() {
  global $current_user;
  $current_user_id   = $current_user->ID;
  $theme  = wp_get_theme();
  if ( !get_user_meta( $current_user_id, esc_html( $theme->get( 'TextDomain' ) ) . '_notice_ignore' ) ) {
    ?>
    <div class="notice freesia-business-notice" style="position:relative;">

      <h1>
        <?php
        /* translators: %1$s: theme name, %2$s theme version */
        printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'freesia-business' ), esc_html( $theme->Name ), esc_html( $theme->Version ) );
        ?>
      </h1>
      ...
      <p>
        ...
        printf( __( 'Welcome! Thank you for choosing %1$s! View  <a href="%2$s" target="_blank" class="notice-links">Multiple Demos</a> <a href="%3$s" target="_blank" class="notice-links">Documentation</a>', 'freesia-business' ), esc_html( $theme->Name ), 'https://themefreesia.com/demos/freesia-empire-demos/', 'https://themefreesia.com/theme-instruction/freesia-business/' );
      ...
      <p>
        <a href="https://themefreesia.com/theme-freesia-demo-import/"  target="_blank" class="button" style="text-decoration: none;">
          ...
          printf( esc_html__( 'Download Demo Import Plugin %s', 'freesia-business' ), esc_html( $theme->Name ) )
          ?>
        </a>
      </p>
    </div>
...
add_action( 'admin_notices', 'freesia_business_admin_notice' );

function freesia_business_notice_ignore() {
  global $current_user;
...
add_action( 'admin_init', 'freesia_business_notice_ignore' );