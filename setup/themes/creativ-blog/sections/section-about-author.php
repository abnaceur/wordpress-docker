 ...
 */
    ...
    $ss_content_type     = creativ_blog_get_option( 'ss_content_type' );
    $number_of_ss_column = creativ_blog_get_option( 'number_of_ss_column' );
    $number_of_ss_items  = creativ_blog_get_option( 'number_of_ss_items' );

    if( $ss_content_type == 'ss_page' ) :
        for( $i=1; $i<=$number_of_ss_items; $i++ ) :
            $about_author_posts[] = creativ_blog_get_option( 'about_author_page_'.$i );
        endfor;  
    elseif( $ss_content_type == 'ss_post' ) :
        for( $i=1; $i<=$number_of_ss_items; $i++ ) :
            $about_author_posts[] = creativ_blog_get_option( 'about_author_post_'.$i );
        endfor;
    endif;
    ?>
