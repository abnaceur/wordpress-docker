...
if ( ! function_exists( 'creativ_blog_get_default_theme_options' ) ) :
	 ...
	 */
function creativ_blog_get_default_theme_options() {
	...
	//General Section
	$defaults['readmore_text']					= esc_html__('Keep reading','creativ-blog');
	...
	//Footer section 		
	$defaults['copyright_text']				= esc_html__( 'Copyright &copy; All rights reserved.', 'creativ-blog' );

	// Pass through filter.
	$defaults = apply_filters( 'creativ_blog_filter_default_theme_options', $defaults );
	return $defaults;
}
...
*  Get theme options
*/
if ( ! function_exists( 'creativ_blog_get_option' ) ) :
	 ...
	 */
	function creativ_blog_get_option( $key ) {

		$default_options = creativ_blog_get_default_theme_options();
		if ( empty( $key ) ) {
			return;
		}
