        ...
        foreach ($results as $v) {
            ...
            if (!empty($v->meta)) {
                ...
                if (!empty($meta->question)) {
                    ...
                    $details = 'Q: '.$question.'<br>A: '.$answer;
                }else{
            ...
            if (! empty($_GET['ids'])) {
                ...
                $result = $wpdb->query($safe_sql);
                    ...
                    foreach($ids as $id){
                        ...
                        $tablename = $wpdb->prefix . 'rafflepress_entries';
                        $sql = "UPDATE " . $tablename . " SET deleted_at = NULL WHERE action_id = %s AND contestant_id = %d";
                        $safe_sql = $wpdb->prepare($sql, $result->action_id, $result->contestant_id);
                        $result = $wpdb->query($safe_sql);
                    }
            }
        ...
        if (current_user_can( apply_filters('rafflepress_list_users_capability', 'list_users') )) {
            if (! empty($_GET['ids'])) {
                ...
                foreach($ids as $id){
                    ...
                    $tablename = $wpdb->prefix . 'rafflepress_entries';
                    $sql = "UPDATE " . $tablename . " SET deleted_at = now() WHERE action_id = %s AND contestant_id = %d";
                    $safe_sql = $wpdb->prepare($sql, $result->action_id, $result->contestant_id);
                    $result = $wpdb->query($safe_sql);
                }
            }
