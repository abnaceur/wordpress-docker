...
function rafflepress_lite_giveaway_api()
{
    ...
    if ($method == 'return') {
            ...
            foreach ($entry_options as $k=>$v) {
                if (!empty($v->type) && $v->type == 'join-newsletter') {
                    if (!empty($v->auto_trigger)) {
                        ...
                        rafflepress_lite_process_email(array(
                                'contestant_id'=>$contestant['id'],
                                'action_id'=>$v->id,
        ...
        if (empty($entry_count)) {
            ...
            do_action('rafflepress_post_entry_add', array('giveaway_id'=>$giveaway_id,'contestant_id'=>$contestant_id,'action_id'=>$action_id,'entry_option_meta'=>$entry_option_meta));
        ...
        if (!@getimagesize($_FILES['file']['tmp_name'])) {
            $errors[] = __('Invalid Image', 'raffelpress_pro');
        } elseif ($_FILES['file']['size'] > 5242880) { //5 MB (size is also in bytes)
            $errors[] = __('Image too large. Must be under 5MB', 'raffelpress_pro');
        } else {
            $errors = null;
            $allowed_mimes = array(
            ...
            if (!empty($file_info['type'])) {
                ...
                $upload_info = wp_handle_upload($_FILES['file'], array(
                        'test_form' => false,
                        'mimes'     => $allowed_mimes,
                    ));
...
insertIframe( '<?php echo $iframe_uid; ?>',
        '<?php
            echo trailingslashit(home_url())
            .'?rafflepress_page=rafflepress_render&rafflepress_id='
            .$id.'&iframe=1&giframe='.$a['giframe'].'&rpr='.$ref
            .'&parent_url='.urlencode($parent_url); ?>&<?php echo mt_rand(1, 99999);
        ?>&rp-email='+rafflepress_getParameterByName('rp-email',location.href)+'&rp-name='+rafflepress_getParameterByName('rp-name',location.href),
        '<?php echo $a['min_height']; ?>' );
</script>
...
function rafflepress_lite_display_gutenberg_shortcode($atts)
{
    ...
    if (!empty($a['min_height'])) {
        $style = 'style="min-height:'.$a['min_height'].'"';
    } ?>
