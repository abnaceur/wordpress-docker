<?php
    ...
    if (empty($giveaway)) {
        wp_die(__('No Giveaway Found','rafflepresss-pro'));
    }
    ...
    if (!empty($_GET['confirm']) && !empty($_GET['id'])) {
        ...
        if (!empty($contestant)) {
            ...
            if ($contestant_confirmed) {
                ...
                setcookie('rafflepress_flash_'.$rafflepress_id, urlencode($msg), strtotime("+1 year"), '/');
                setcookie('rafflepress_hash_'.$rafflepress_id, urlencode($contestant->id.'|'.$contestant->email), strtotime("+1 year"), '/');
                setcookie('rafflepress_email', $contestant->email, strtotime("+1 year"), '/');
            }
    ...
    if (!empty($settings->entry_options)) {
        foreach ($settings->entry_options as $v) {
            ...
            if ($v->type== 'fb-page-post' || $v->type== 'visit-fb') {
                $include_fb_sdk = true;
            }
            if ($v->type== 'instagram-page-post' && !empty($v->post_url)) {
...
<body style="padding-top:0">
  ...
  align-items: center;">
    ...
    <body>
        ...
        <?php if ($include_instagram_sdk) {
        ?>
        <script async src="//www.instagram.com/embed.js"></script>
        <?php } ?>

        <?php if ($include_fb_sdk) {
        ?>
        <div id=" fb-root"></div>
        <script>
