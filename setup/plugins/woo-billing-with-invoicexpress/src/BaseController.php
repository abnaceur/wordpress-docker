...
class BaseController {
		...
		if ( ! $no_values ) {
			...
			if ( ! empty( $shipping_method ) ) {
				foreach ( $order_object->get_shipping_methods() as $key => $item ) {
					...
					//if ( $cost > 0 ) { //We will prevent ir later
			...
			foreach ( $order_object->get_fees() as $key => $item ) {
				...
				if ( $no_values ) {
					...
					$fee_price = (double) $item['line_total'];
					//Fee refunds?
					foreach ( $order_object->get_refunds() as $refund ) {
				...
				//if ( $fee_price > 0 ) { //We will prevent ir later
					$taxes_per_line = $item['taxes'];
					if ( $taxes_per_line && ! is_array( $taxes_per_line ) ) {
						$taxes_per_line = unserialize( $taxes_per_line );
					}
	...
	public function storeAndNoteDocument( $order_object, $document_url, $type, $invoicexpress_id, $another_doc = '' ) {
		...
		$file_name = $type_name.'-'.$order_object->get_meta( 'hd_wc_ie_plus_'.$type.'_sequence_number' );
	...
	public function storeAndNoteMassDocument( $order_array, $document_url, $type, $invoicexpress_id, $sequence_number, $another_doc = '' ) {
		...
		$file_name = $type_name.'-'.$order_array[0]->get_meta( 'hd_wc_ie_plus_'.$type.'_sequence_number' );
		$file_name .= '-'.substr( md5( $file_name.time() ), 0, 5 );
		$file_name = sanitize_title( $file_name ).'.pdf';
	...
	public function getAndSendPDF( $order_object, $type, $order_id_invoicexpress, $mode = 'manual', $receipt_count = 1 ) {
		if ( get_option( 'hd_wc_ie_plus_email_method' ) != '' && apply_filters( 'invoicexpress_woocommerce_allow_ix_email', true ) ) {
			...
			//Force fail to test pending documents screen
			...
			if ( ! $return['success'] ) {
				...
				if ( $error_notice ) {
					...
					$not_finalized[$order_object->get_id()][$type] = 1;
				...
				$not_finalized = get_option( 'hd_wc_ie_plus_pending_email', array() );
				if ( isset( $not_finalized[$order_object->get_id()][$type] ) ) {
					unset( $not_finalized[$order_object->get_id()][$type] );
	...
	*                       - invoice_receipt
	...
	public function sendErrorEmail( $order_object, $error_message ) {
		...
		$email_address = get_option( 'hd_wc_ie_plus_automatic_email_errors_address', get_option( 'admin_email' ) ); //Default to the admin email if not set

		wc_mail( $email_address, $subject, $message, $headers );
	...
	public function finalize_document( $order_object, $document_type, $mode ) {
		...
		if ( $order_id_invoicexpress = $order_object->get_meta( 'hd_wc_ie_plus_'.$document_type.'_id' ) ) { //Check if $document_type is valid for all...
			...
			/* Save */
			$sequence_number = $return['object']->$document_type->inverted_sequence_number; //Check if $document_type is valid for all...
