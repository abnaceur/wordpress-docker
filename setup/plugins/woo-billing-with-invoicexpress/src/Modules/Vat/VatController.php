...
class VatController {
	...
	public function register_hooks() {
		...
		add_action( 'after_setup_theme', function() {
			...
			if ( get_option( 'hd_wc_ie_plus_vat_field' ) ) {
				...
				add_filter( 'woocommerce_api_customer_billing_address', array( $this, 'api_customer_billing_address_fields' ), 10, 1 );

				add_filter( 'woocommerce_api_customer_response', array( $this, 'api_customer_response' ), 10, 2 );
			...
			// Order observations.
			add_action( 'woocommerce_admin_order_data_after_shipping_address', array( $this, 'add_order_observations' ), 12 );
	...
	public function custom_billing_fields( $fields ) {
		global $post;
		if ( $order_object = wc_get_order( $post->ID ) ) {
	...
	public function api_customer_billing_address_fields( $fields ) {
		$fields[] = 'VAT_code';
		return $fields;
	}

	public function api_customer_response( $customer_data, $customer ) {
	...
	public function ajax_get_customer_details( $data, $customer, $user_id ) {
		$data['billing']['VAT_code'] = get_user_meta( $user_id, 'billing_VAT_code', true );
		return $data;
	}
